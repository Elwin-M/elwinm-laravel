<div class="relative border border-gray-200 rounded-xl dark:border-gray-800 group">
    <div
        ...
        {{-- {{ $attributes->merge(['class' => 'absolute -inset-px rounded-xl border-2 border-transparent opacity-0 [background:linear-gradient(var(--quick-links-hover-bg,theme(colors.sky.50)),var(--quick-links-hover-bg,theme(colors.sky.50)))_padding-box,linear-gradient(to_top,theme(colors.indigo.400),theme(colors.cyan.400),theme(colors.sky.500))_border-box] group-hover:opacity-100 dark:[--quick-links-hover-bg:theme(colors.slate.800)]']) }} --}}
    ...
    <div class="relative p-6 overflow-hidden rounded-xl">
        ...
        <h2 class="mt-4 text-base text-gray-900 font-display dark:text-white">
            ...
            @if (isset($linkText))
                <a
                    wire:click="changeContent('{{ $menu }}', '{{ $item }}'), activePage = 'pages.about.long'">
